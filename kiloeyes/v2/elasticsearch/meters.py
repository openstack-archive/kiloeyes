    ...
    def get_meter(self, req, res):
        ...
        if res_data:
            ...
            def _render_hits(item):
                # TODO: confirm if _id read should be meter_id
                _id = item['meters']['hits']['hits'][0]['_id']
                _type = item['meters']['hits']['hits'][0]['_type']
                _source = item['meters']['hits']['hits'][0]['_source']
                _index = item['meters']['hits']['hits'][0]['_index']
                # project_id, user_id, user_agent, tenant_id are not included
                # in _source data of length smaller than 9
                if len(_source) == 9:
                    project_id = json.dumps(_source['project_id'])
                    tenant_id = json.dumps(_source['tenant_id'])
                    user_id = json.dumps(_source['user_id'])
                    user_agent = json.dumps(_source['user_agent'])
                else:
                    project_id = 'null'
                    tenant_id = 'null'
                    user_id = 'null'
